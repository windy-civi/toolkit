...
if docker run --rm \
  ...
  -c "/root/.cache/pypoetry/virtualenvs/*/bin/pip install sqlalchemy pymysql && /root/.cache/pypoetry/virtualenvs/*/bin/os-update ca bills --scrape --fastmode" 2>&1 | tee -a "$SCRAPE_LOG"
then
